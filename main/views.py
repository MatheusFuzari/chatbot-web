...
class ChatBotAPIView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(status=201, data={'content': "Usuário não existe!"})
        ...
        if(conversationId is None):
            ...
            try:
                conversationExistis = Conversation.objects.get(pk=conversationId)
                print(conversationExistis)
            except ObjectDoesNotExist:
                return JsonResponse(status=201, data={'content': "Conversa não existe!"})
        newQuestion = Conversation(type="q", message=question, history=conversationExistis)
        newQuestion.save()
        
        answer = chat.get_response(question)
               
        newAnswer = Conversation(type="a", message=answer.message, history=conversationExistis)
        newAnswer.save()
               
        serializerAnswer = ConversationSerializer(newAnswer, many=False)
               
        return JsonResponse(status=201, data={'content': serializerAnswer.data})

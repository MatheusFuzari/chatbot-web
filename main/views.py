...
class ScoreAverageView(APIView):
    ...
    def get(self, request, tripId=''):
        if tripId == '':
            return Response(status=400, data='Missing required parameter tripId')
        try:
            tripFound = Trip.objects.get(id=tripId)
            serializerTrip = TripSerializer(tripFound, many=False)
            average = Booking.objects.filter(
                tripFK=tripId).aggregate(Avg('score'))
            if average is not None:
                return Response(status=200, data={'average': average['score__avg'], 'trip': serializerTrip.data})
            else:
                return Response(status=404, data='Getting Average of this trip is not possible')
...
class BookingView(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        for dateChosen in dayList:
            ...
            availability = Availability.objects.filter(tripFK=data['tripFK']).filter(
                date=dateChosen).filter(bookingFK=None).first()
        ...
        savedBookingResponse = super(
            BookingView, self).create(request, *args, **kwargs)
        if savedBookingResponse.status_code == 201:
            savedBooking = Booking.objects.get(
                pk=savedBookingResponse.data['id'])
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            queryset = Payment.objects.filter(
                bookingFK__customUserFK__user__username=user.username)
        return queryset
...
class ChatBotAPIView(APIView):
    def post(self, request):
        ...
        userExistis = None
        try:
            userExistis = User.objects.get(pk=userId)
        except ObjectDoesNotExist:
            return JsonResponse(status=500, data={'content': "Usuário não existe!"})

        conversationExistis = None

        if conversationId is None:
            newHistory = ConversationHistory(user=userExistis)
            newHistory.save()
            conversationExistis = newHistory
        else:
            try:
        ...
        newQuestion = Conversation(
            type="Q", message=question, history=conversationExistis)
        newQuestion.save()

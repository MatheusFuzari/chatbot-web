...
const { data: historyConv } = await useFetch(`http://127.0.0.1:8000/chatbot/${routeId}`)
historyConv._value.forEach(element => {
  console.log(element.message)
  dialog.text = element.message
  dialog.historyId = element.history.id
  includeDialog(`${element.type}`);
  dialog.text = '';
});
</script>
      ...
      <div class="bg-zinc-900 w-[calc(100vw-32rem)] h-[calc(100vh-6rem)] text-white rounded-lg">
        ...
        <div class="bottom-0 absolute">
          ...
          <Button @click="sendMessage"
            class="w-14 h-14 bg-slate-800 rounded-3xl items-center text-center justify-center fixed bottom-[calc(1vh+5.5rem)] left-[calc(100vw-14rem)]">
